...
| --- | --- |
| **v2.3.2408** | <ul><li>**Pi project providers migrate to readable names**: projects taken over before 2.3.2405 kept a bare-hash `ccr-project-&lt;hash&gt;` provider that the refresh path skipped; they now migrate to `ccr-project-&lt;project&gt;-&lt;hash&gt;` with the orphan entry removed, and disabling the takeover clears the legacy entry too.</li><li>**Tests no longer write into the live usage database**: fixture requests (demo/global/project providers) used to land in the real `usage.sqlite`; the store now honors CCR_CONFIG_DIR.</li><li>**npm package pages link back to the repository** (repository/homepage/bugs and author metadata).</li></ul> |
| **v2.3.2407** | <ul><li>**ZCode session discovery**: ZCode requests carry no project identity, so project-level Routers never applied to them. CCR now maps a session back to the directory it ran in via ZCode's local task index (`~/.zcode/v2/tasks-index.sqlite`, with the session JSON's `meta.workspacePath` as fallback) and resolves it to the same project id `ccr model --project` and the UI use; a late-written index row gets one short retry, and any failure degrades to the global Router.</li><li>**ZCode as a project takeover client (opt-in)**: The project's "Clients to take over" list now includes ZCode, and that selection is the routing switch — only projects that select ZCode route ZCode sessions through their project Router, while unselected projects keep the global Router, so session discovery can never redirect a personal project to project/company models. ZCode has no writable project-scoped config file, so the marker lives in `~/.claude-code-router/<project-id>/takeover-clients.json` and is cleared when ZCode is deselected or the project is removed.</li></ul> |
| **v2.3.2406** | <ul><li>**Custom values for the default thinking level**: Besides the enum, `default_thinking_level` now accepts a token budget or a provider-specific string (e.g. `minimal`/`off`). Numbers inject an exact budget on Anthropic endpoints and collapse to the closest level on OpenAI-style ones; non-enum strings pass through verbatim on OpenAI-style endpoints and resolve to the level they stand for on Anthropic endpoints (min/minimal/light→low, xhigh/maximum/ultra→max). The UI gains a free-text "Custom" option.</li><li>**Thinking depth is published as effort**: `convertToAnthropic` now sends `output_config.effort` (the field current Anthropic models and Anthropic-compatible endpoints such as GLM actually read) alongside `thinking.budget_tokens`, which stays for budget-era models and relays. A client's own effort is no longer dropped, `thinking.type=adaptive` is no longer read as thinking-off, and an exact budget is no longer rounded to a tier.</li><li>**ZCode attributed as its own client**: ZCode used a `ccr-*` alias with Claude Code compatible metadata, so it was logged as claude-code; it is now detected from its request headers (`User-Agent: ZCode/…`, `X-ZCode-*`) and reported separately.</li><li>**release.sh resumes after an interrupted publish**: a rerun skips already-published packages and ships only the rest.</li></ul> |
