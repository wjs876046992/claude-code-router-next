...
| **v2.3.13** | <ul><li>**Fix Project Deletion Leaving ccr Fields in `settings.local.json`**: Adding a project auto-writes ccr-managed fields (proxy URL, model family routing env vars, auto-compact, status line) into the project's `.claude/settings.local.json`, but deleting the project previously left them behind; deletion now disables takeover first to remove those fields.</li></ul> |> Only the latest 10 versions are kept here. Older release summaries are archived in [CHANGELOG-archive.md](./CHANGELOG-archive.md); the full detailed changelog is in [CHANGELOG.md](./CHANGELOG.md).
