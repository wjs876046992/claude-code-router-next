...
| --- | --- |
| **v2.3.9** | <ul><li>**Fix Runtime Fallback Retry Ignoring Project-Level `enableFallback`**: When an in-flight request failed (e.g. rate limit) and triggered a retry fallback, it previously read the global `Router.enableFallback` and global top-level `fallback` config directly, ignoring a project's `enableFallback: false` and custom `Router.fallback`. The runtime retry now uses the same project-resolved fallback config as the routing decision.</li></ul> |
