...
| 版本 | 发布内容 |
| --- | --- |
| **v2.3.23** | <ul><li>**修复状态栏 token 速率虚高（撞到 999 上限）**: `ccr statusline` 的 token 速率常显示几百、极端撞到 999 上限，与「用量统计」的几十 t/s 对不上。根因是 token-speed 插件流式过程中每秒上报的是「最近 1 秒到达的 token 数」滑动窗口值，而 SSE delta 常成批到达（同一时间戳）使瞬时计数虚高；现改为与用量统计同一套解码平均公式（`输出 token ÷ (总耗时 − TTFT)`）上报，速率随流式收敛到最终值、与用量统计一致，并移除失效的滑动窗口记账。</li></ul> |
...
| **v2.3.14** | <ul><li>**修复状态栏显示 `<synthetic>` 模型名**：Claude Code 在 auto-compact 自动压缩、中断恢复时写入的 `<synthetic>` 合成消息（非真实 LLM 响应）此前被状态栏当作模型名显示（从 Claude 账号会话切换到 ccr 接管、或发生自动压缩后尤其常见）；现在过滤该合成标识，正确显示实际路由模型，同时不再把合成消息的 usage 计入 token 统计。</li></ul> |> 仅保留最近 10 个版本，更早版本的发布摘要见 [CHANGELOG-archive.md](./CHANGELOG-archive.md)，完整详细变更记录见 [CHANGELOG.md](./CHANGELOG.md)。
